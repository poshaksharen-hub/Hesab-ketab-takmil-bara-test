...
-- Version: 2.0 (for Supabase CLI)
-- ====================================================================
-- This script contains all necessary SQL commands to set up the 
-- database from scratch. It is designed to be run via `supabase db push`.
-- ====================================================================
...
END;
$$;
COMMENT ON FUNCTION public.create_check IS 'Creates a new cheque with "pending" status and blocks the amount in the bank account.';
...
END;
$$;
COMMENT ON FUNCTION public.clear_check IS 'Atomically clears a cheque, creates an expense, and updates balances.';
...
END;
$$;
COMMENT ON FUNCTION public.delete_check IS 'Safely deletes a non-cleared cheque and reverts the blocked balance.';
...
AS $$
    SELECT id, email, raw_user_meta_data->>'first_name' as first_name, raw_user_meta_data->>'last_name' as last_name, raw_user_meta_data->>'signature_image_path' as signature_image_path FROM auth.users;
$$;
COMMENT ON FUNCTION public.get_all_users() IS 'Securely fetches a list of all user profiles from auth.users.';
