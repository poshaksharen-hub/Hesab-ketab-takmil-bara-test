...
export default function LoansPage() {
 ...
 const handleFormSubmit = useCallback(async (values: any) => {
    ...
    if (editingLoan) {
        ...
        const operations = [setDoc(newLoanRef, loanData)];
        ...
        if (loanData.depositOnCreate && loanData.depositToAccountId) {
            ...
            operations.push(
                getDoc(bankAccountRef).then(bankAccountDoc => {
                    ...
                    return updateDoc(bankAccountRef, { balance: balanceAfter });
                })
            );
        }
        
        Promise.all(operations).then(async () => {
