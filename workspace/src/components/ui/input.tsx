...
const CurrencyInput = React.forwardRef<HTMLInputElement, CurrencyInputProps>(
  ({ className, value, onChange, ...props }, ref) => {
    ...
    React.useEffect(() => {
        // This effect synchronizes the displayValue with the external `value` prop
        // when the prop changes from outside the component.
        ...
        // Convert current display value back to a raw number string for comparison
        const rawDisplayNumber = toEnglishDigits(displayValue).replace(/,/g, '');

        if (numericValue.toString() !== rawDisplayNumber) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setDisplayValue(rawValue === '' ? '' : new Intl.NumberFormat('fa-IR').format(Number(rawValue)));
